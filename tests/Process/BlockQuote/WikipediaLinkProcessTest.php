...
MARKDOWN,
    ...
    {
        $actual = (new WikipediaLinkProcess(new CachingUrlFetcher()))->shouldProcess($block);
    ...
    {
        $actual   = (new WikipediaLinkProcess(new CachingUrlFetcher()))
            ->processBlockQuote(self::TEST_MARKDOWN_NO_HIGHLIGHT);
...
> [https://w.wiki/wor](https://en.wikipedia.org/w/index.php?title=Data_type)';
    ...
    {
        $actual   = (new WikipediaLinkProcess(new CachingUrlFetcher()))
            ->processBlockQuote(self::TEST_MARKDOWN);
        $expected = '> #### Data type - Wikipedia
